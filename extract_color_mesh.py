...
def get_opts():
    ...
    parser.add_argument('--use_vertex_normal',
                        action="store_true",
...
if __name__ == "__main__":
    ...
    W, H = args.img_wh
    K = np.array([[dataset.focal, 0, W / 2], [0, dataset.focal, H / 2],
    ...
    if args.use_vertex_normal:  ## use normal vector method as suggested by the author.
        ...
        results = f([nerf_coarse, nerf_fine], embeddings,
                    torch.cat([rays_o, rays_d, near, far],
                              1).cuda(), args.N_samples, args.N_importance,
        ...
        for idx in tqdm(range(len(dataset.image_paths))):
            ...
            depth = vertices_image[:,
                                   -1:] + 1e-5  # the depth of the vertices, used as far plane
            ...
            rays_o = torch.FloatTensor(dataset.poses[idx][:, -1]).expand(
                N_vertices, 3)
            ...
            results = f([nerf_fine], embeddings,
                        ...
                        args.N_samples, 0, args.chunk, dataset.white_back)
            opacity = results['opacity_coarse'].cpu().numpy(
            )[:, np.newaxis]  # (N_vertices, 1)
            opacity = np.nan_to_num(opacity, 1)

            non_occluded = np.ones_like(
                non_occluded_sum) * 0.1 / depth  # weight by inverse depth
    ...
    vertices_.dtype = [('x', 'f4'), ('y', 'f4'), ('z', 'f4')]
    vertex_all = np.empty(N_vertices,
                          vertices_.dtype.descr + v_colors.dtype.descr)
    ...
    PlyData([
        PlyElement.describe(vertex_all, 'vertex'),
        PlyElement.describe(face, 'face')
    ]).write(f'{args.scene_name}.ply')
