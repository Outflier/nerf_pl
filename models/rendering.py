...
def render_rays(models,
    ...
    def inference(model,
        ...
        if not weights_only:
            dir_embedded = torch.repeat_interleave(dir_embedded,
                                                   repeats=N_samples_,
                                                   dim=0)
        ...
        alphas = 1 - torch.exp(
            -deltas * torch.relu(sigmas + noise))  # (N_rays, N_samples_)
        alphas_shifted = \
            torch.cat([torch.ones_like(alphas[:, :1]), 1-alphas+1e-10], -1) # [1, a1, a2, ...]
        weights = \
            alphas * torch.cumprod(alphas_shifted, -1)[:, :-1] # (N_rays, N_samples_)
        weights_sum = weights.sum(
            1)  # (N_rays), the accumulated opacity along the rays
    ...
    if N_importance > 0:  # sample points for fine model
        ...
        z_vals_ = sample_pdf(z_vals_mid,
                             weights_coarse[:, 1:-1],
                             N_importance,
                             det=(perturb == 0)).detach()
